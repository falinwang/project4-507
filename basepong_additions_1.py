...
#3) Keep score
#4) Prevent the paddle from going off the top or bottom of the screen
    # (HINT: add something to Paddle's update method)
#5) Change the images to curved paddles
    # new images provided: bowright.png; bowleft.png (still needed)
#6) Change the deflection to match the curved paddles
    # (change Paddle's deflect_ball method so it doesn't call BallDeflector's version anymore; implement it differently)
#7) Create a maze of walls with bricks
    # (in __init__ for Game class)
...
class Paddle (BallDeflector):
    ...
    def update(self,pressed_keys):
        ...
        self.prevent_offscreen()

    def prevent_offscreen(self):
        self.y = max(0, self.y)
        self.y = min(self.game.height - self.height, self.y)
    ...
    def deflect_ball(self, ball, side_hit):
        '''
        Deflect a ball that has collided with this paddle.
        For paddles that bulge out in the middle.
        :param ball:
        '''

        if side_hit == 'RIGHT':
            # set the angle to be in the range (-75, +75)
            # close to +80 if hit near the top of the paddle
            ball.angle = (-75 + 150*self.hit_position(ball)) % 360
        elif side_hit == 'LEFT':
            # set the angle to be in the range (105, 245)
            # close to 100 if hit near the top;
            ball.angle = (245 - 150*self.hit_position(ball) ) % 360
        elif side_hit == 'BOTTOM' or side_hit == 'TOP':
            ball.angle = (- ball.angle) % 360
...
class Game(object):
    ...
    def __init__(self,
        ...
        for i in range(5):
            self.bricks.append(BallDeflector(
                initial_x = 400,
                initial_y = self.height - i*brick_height,
                img_file = wall_imgs[2],
                game = self))
